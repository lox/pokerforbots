...
// flushTable and CHD structures are generated by gen_perfect_hash.go.
...
var unsuitedCHDData []byte    // Serialized CHD structure
var unsuitedValues []HandRank // Values indexed by CHD lookup
var unsuitedCHD *chd.Chd      // Deserialized CHD for lookups

// perfectHashReady indicates whether go:generate has produced the tables.
var perfectHashReady bool

var chdInitOnce sync.Once

// ensureCHDLoaded ensures CHD is deserialized exactly once
func ensureCHDLoaded() {
	chdInitOnce.Do(func() {
		// Deserialize CHD if data is available
		if len(unsuitedCHDData) > 0 && unsuitedCHD == nil {
			unsuitedCHD = &chd.Chd{}
			err := unsuitedCHD.UnmarshalBinaryMmap(unsuitedCHDData)
			if err != nil {
				// Fall back to legacy evaluator if CHD fails to load
				log.Printf("CHD deserialization failed: %v", err)
				unsuitedCHD = nil
			}
		}

		perfectHashReady = len(flushTable) > 0 && (unsuitedCHD != nil && len(unsuitedValues) > 0)
	})
}

// unsuitedLookupCHD performs minimal perfect hash lookup using CHD.
// Target performance: 5-6ns (2 table reads, 1 multiply, no branches)
//
//go:noinline
func unsuitedLookupCHD(primeProd uint64) HandRank {
	...
	if index >= uint64(len(unsuitedValues)) {
		// Index out of bounds - shouldn't happen with valid CHD
		log.Printf("CHD index out of bounds: %d >= %d for key %d", index, len(unsuitedValues), primeProd)
		return Evaluate7Legacy(nil) // This will cause a panic, but better than silent corruption
	}

	result := unsuitedValues[index]
	if result == 0 {
		// This should never happen with a working CHD for our key set
		panic(fmt.Sprintf("CHD returned zero value for key %d at index %d - regenerate tables", primeProd, index))
	}

	return result
}

// Evaluate7Perfect evaluates a 7-card hand using CHD minimal perfect hash.
// This implementation targets 5-6ns for unsuited hands via CHD lookup.
//
// OPTIMIZATION PROGRESSION:
// - Map-based lookup: ~35ns - Go's optimized hash tables
// - Optimized bucketed linear scan: ~40ns - Custom buckets with 95.2% ≤64 keys
// - CHD minimal perfect hash (current): ~5-6ns target - 2 table reads, 1 multiply
...
func Evaluate7Perfect(cards []deck.Card) HandRank {
	...
	return unsuitedLookupCHD(primeProd)
}
