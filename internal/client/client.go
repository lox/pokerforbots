...
func (c *client) handleHandStart(msg *protocol.HandStart) error {
	...
	if msg.YourSeat >= 0 {
		roles[msg.YourSeat] = appendRole(roles[msg.YourSeat], "you")
	}
...
func (c *client) handlePlayerAction(msg *protocol.PlayerAction) error {
	...
	if state != nil && state.id == msg.HandID && msg.Seat >= 0 && msg.Seat < len(state.players) {
		...
		state.pot = msg.Pot
		nameLabel = formatActionName(state, msg.Seat, player.Name, player.Folded, player.AllIn)
		...
		nameLabel = formatActionName(nil, msg.Seat, msg.PlayerName, false, false)
	}
	c.mu.Unlock()
...
func (c *client) handleActionRequest(req *protocol.ActionRequest) error {
	...
	position := ""
	heroStack := -1
	if state != nil && state.yourSeat >= 0 && state.yourSeat < len(state.players) {
		position = positionAbbrev(state, state.yourSeat)
		heroStack = state.players[state.yourSeat].Chips
	}
	c.renderActionDialog(req, state, position, heroStack)
	return nil
}

func (c *client) renderActionDialog(req *protocol.ActionRequest, state *handState, position string, heroStack int) {
	stdoutln("")
	lines := make([]string, 0, 5)
	lines = append(lines, colorize("=== Your action ===", colorBold+colorCyan))

	board := formatCards(nil)
	hole := formatCards(nil)
	if state != nil {
		board = formatCards(state.board)
		hole = formatCards(state.holeCards)
	}
	if position != "" {
		hole += fmt.Sprintf(" (%s)", position)
	}
	lines = append(lines, fmt.Sprintf("Board %s  Hole %s", board, hole))
	lines = append(lines, fmt.Sprintf("Call %s  Min raise-to %s  Pot %s", formatAmount(req.ToCall), formatAmount(req.MinBet), formatAmount(req.Pot)))
	stackText := "--"
	if heroStack >= 0 {
		stackText = formatAmount(heroStack)
	}
	lines = append(lines, fmt.Sprintf("Stack %s  Time %s", stackText, formatMillis(req.TimeRemaining)))
	lines = append(lines, fmt.Sprintf("Choices: %s", formatActionChoices(req.ValidActions)))

	for _, line := range lines {
		stdoutln(line)
	}
	c.mu.Lock()
	c.actionPromptLines = len(lines) + 2
	c.mu.Unlock()
	c.printPrompt()
}
...
func (c *client) printPlayers(label string) {
	...
	for i, p := range c.state.players {
		...
		name := formatActionName(c.state, i, p.Name, p.Folded, p.AllIn)
...
func formatActionName(state *handState, seat int, name string, folded, allIn bool) string {
	display := fallbackName(name, seat)
	var base string
	switch {
	case folded:
	...
	if pos := positionAbbrev(state, seat); pos != "" {
		return base + colorize(fmt.Sprintf(" (%s)", pos), colorDim)
	}
	return base
}

func positionAbbrev(state *handState, seat int) string {
	...
	if pos, ok := positionsForState(state)[seat]; ok {
		return pos
	}
	return ""
}

func positionsForState(state *handState) map[int]string {
	positions := make(map[int]string)
	if state == nil || len(state.players) == 0 {
		return positions
	}
	for offset := 0; offset < len(state.players); offset++ {
		seat := (state.button + offset) % len(state.players)
		positions[seat] = shortPositionLabel(offset)
	}
	return positions
}

func shortPositionLabel(offset int) string {
	labels := []string{"BTN", "SB", "BB", "UTG", "LJ", "HJ", "CO"}
...
func formatActionChoices(actions []string) string {
	if len(actions) == 0 {
		return ""
	}
	optionList := append([]string(nil), actions...)
	hasAggressiveOption := false
	hasAllin := false
	for _, act := range actions {
		if act == "raise" || act == "call" || act == "bet" {
			hasAggressiveOption = true
		}
		if act == "allin" {
			hasAllin = true
		}
	}
	if hasAggressiveOption && !hasAllin {
		optionList = append(optionList, "allin")
	}
	formatted := make([]string, len(optionList))
	for i, action := range optionList {
		formatted[i] = formatActionChoice(action)
	}
	return strings.Join(formatted, ", ")
}

func formatActionChoice(action string) string {
	switch action {
	case "fold":
		return colorize("(f)old", colorRed)
	case "check":
		return colorize("(x)heck", colorBlue)
	case "call":
		return colorize("(c)all", colorGreen)
	case "bet":
		return colorize("(b)et", colorMagenta+colorBold)
	case "raise":
		return colorize("(r)aise", colorMagenta+colorBold)
	case "allin":
		return colorize("allin", colorMagenta+colorBold)
	default:
		return colorize(action, colorBold)
	}
}
