...
- cmd/pokerforbots/solver.go – CLI surface for training/evaluation and configuration flow.
...
- Step 3: Add `cmd/pokerforbots solver train` subcommand to orchestrate self-play runs via the PokerForBots engine and persist blueprint packs.
- Step 4: Add `cmd/pokerforbots solver eval` subcommand for mirror-mode matches, exploitability probes, and BB/100 reporting.
...
- Checkpoint/resume support stores trainer state to JSON snapshots; CLI now exposes `--checkpoint-path`, `--checkpoint-every`, and `--resume-from` for iterative local runs (sdk/solver/checkpoint.go:1, cmd/pokerforbots/solver.go:1).
...
- Trainer plumbing, blueprint serialization, and CLI stubs now drive a fold/call MCCFR traversal over real hand simulations with node instrumentation emitted via progress hooks (sdk/solver/trainer.go:1, sdk/solver/traversal.go:1, sdk/solver/blueprint.go:1, cmd/pokerforbots/solver.go:1).
...
- Training dry run: `cmd/pokerforbots solver train --iterations=100000 --seed=1 --out=out/blueprint-demo.pb` completes in <15 minutes on an 8-core box.
- Evaluation run: `cmd/pokerforbots solver eval --blueprint=out/blueprint-demo.pb --hands=10000 --mirror` reports BB/100 ±5 with 95% CI.
...
- Exit criteria: `go test ./sdk/solver/...` passes with new fixtures verifying bucket assignments/regret averaging, and a sample blueprint round-trip (`cmd/pokerforbots solver train --iterations=10 --out=out/phase1-smoke.json`) reproduces the expected uniform policy snapshot.
...
- Exit criteria: `cmd/pokerforbots solver train --iterations=5000 --seed=7 --out=out/phase2-blueprint.pb` traverses full-hand simulations (non-zero infosets per street, stable regret deltas) and a toy-game harness asserts exploitability below the analytic baseline.
...
- Exit criteria: `cmd/pokerforbots solver train --iterations=50 --progress-every=1 --disable-raises=false` completes under 2 minutes on a MacBook Air while emitting progress logs each ≤5 seconds, and full-raise training (5k iterations) shows parity against a fold/call-only baseline in evaluation harness smoke tests.

#### Blueprint Smoke Loop

- Run a small local training (e.g., `go run ./cmd/pokerforbots solver train --smoke --iterations=5000 --players=2 --out=dist/h2h.json --checkpoint-path=dist/h2h.ckpt --checkpoint-every=1000`).
...
- [ ] Flesh out `cmd/pokerforbots solver train` with parallel table orchestration, configurable batching, and periodic checkpoint emission gated behind `CheckpointEvery`.
...
- Exit criteria: Running `cmd/pokerforbots solver train --iterations=20000 --parallel=4 --checkpoint-mins=1 --out=out/phase3-blueprint.pb` produces rolling checkpoints, resumes cleanly after interruption (`--resume` or equivalent flag), and blueprint metadata validation rejects tampered files in unit tests.

### Phase 4 – Evaluation Harness & Metrics

- [x] Implement `cmd/pokerforbots solver eval` mirror-mode matches via embedded spawner, reporting BB/100 and perf metrics for solver vs baseline.
...
- Exit criteria: `cmd/pokerforbots solver eval --blueprint=out/phase2-blueprint.pb --hands=10000 --mirror` emits BB/100 ± 95% CI plus exploitability proxy metrics, and CI regression tests flag deviations beyond configured thresholds.
...
bin/task test
bin/task build
cmd/pokerforbots solver train --iterations=1000 --seed=7 --out=out/blueprint-smoke.pb
cmd/pokerforbots solver eval --blueprint=out/blueprint-smoke.pb --hands=2000 --mirror --seed=7
```
